...
$selectQuery = "SELECT * FROM STUDENTS_INSTITUTION_DETAILS_TAB WHERE studentId = ? AND studentProgramId = ?";
$params = [$studentId, $studentProgramId];
...
$selectQuery = "SELECT * FROM STUDENTS_PROGRAM_DETAILS_TAB WHERE studentId = ? AND studentProgramId = ?";
$params = [$studentId, $studentProgramId];
$dataTypes = "ss"; // 'i' for integer, 's' for string, etc.
...
// get payment details
$selectQuery = "SELECT * FROM PAYMENTS_TAB WHERE paymentId= ? AND studentId = ?";
$params = [$paymentId, $studentId];
$dataTypes = "ss"; // 'i' for integer, 's' for string, etc.
...
try {
    ...
    $sendTo = $emailAddress;
    $subject = "$recieverName Payment Receipt - $programName - $courseName - $studentId";
...
<div style="width:100%; background:#f4f6f8; padding:30px 0; font-family:Arial, Helvetica, sans-serif;">
    <div style="max-width:700px; margin:auto; background:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 4px 10px rgba(0,0,0,0.08);">
        ...
        <div style="padding:30px; color:#333;">
            ...
            <h2 style="color:#002B71; margin-top:0;">Payment Receipt</h2>

            <p>Dear <strong>' . $recieverName . '</strong>,</p>

            <p>Your payment for the <strong>' . $programName . '</strong> program has been successfully processed. Below are your payment details:</p>
